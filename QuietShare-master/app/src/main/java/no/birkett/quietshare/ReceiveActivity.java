...
public class ReceiveActivity extends AppCompatActivity {
    ...
    private void subscribeToFrames() {
        ...
        frameSubscription = FrameReceiverObservable.create(this, getProfile()).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(buf -> {
            ...
            long receivedTs = System.currentTimeMillis()-Long.parseLong(receivedContent.getText().toString().split("%")[1]);
            Long time = System.currentTimeMillis() / 1000;
            String timestamp = time.toString();
            double distance = (35 * (receivedTs));

            receiveStatus.setText("Received " + buf.length +"Splitted value ="+receivedContent.getText().toString().split("%")[1]+ " @ distance =" + distance);
